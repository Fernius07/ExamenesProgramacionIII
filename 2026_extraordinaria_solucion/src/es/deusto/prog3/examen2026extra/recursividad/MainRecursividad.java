...
public class MainRecursividad {
    ...
    private static int sumarMinutosRec(List<Actividad> acts, Tipo tipo, Intensidad intensidad, int index) {
        ...
        Actividad actual = acts.get(index);
        int sumaActual = 0;
        if (actual.getTipo() == tipo && actual.getIntensidad() == intensidad) {
            sumaActual = actual.getMinutos();
        }

        return sumaActual + sumarMinutosRec(acts, tipo, intensidad, index + 1);
    }
