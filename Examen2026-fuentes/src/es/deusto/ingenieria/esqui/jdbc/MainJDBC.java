            ...
            if (node.getUserObject() instanceof Pais) {
                ...
                switch (apertura) {
					case ABIERTA:
						...
						label.setText(String.format("%s (%d / %d km.)", e.getNombre(), (int) e.getEstado().getKmEsquiables(), (int) e.getKmEsquiablesTotal()));
						label.setForeground(java.awt.Color.GREEN.darker());
						break;
					case CERRADA:
						label.setIcon(iconoCerrada);
						label.setText(String.format("%s (0 / %d km.)", e.getNombre(), (int) e.getKmEsquiablesTotal()));
						label.setForeground(java.awt.Color.RED.darker());
						break;
					case DESCONOCIDA:
					default:
						label.setIcon(iconoDesconocida);
						label.setText(String.format("%s (?? / %d km.)", e.getNombre(), (int) e.getKmEsquiablesTotal()));
