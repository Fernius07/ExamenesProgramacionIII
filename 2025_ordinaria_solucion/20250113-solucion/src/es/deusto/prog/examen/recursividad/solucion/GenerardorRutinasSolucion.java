...
public class GenerardorRutinasSolucion {
	...
	public static void generarRutinas(List<Ejercicio> ejercicios, int duracion, Nivel nivel, List<GrupoMuscular> gruposMusculares) {
		List<List<Ejercicio>> result = new ArrayList<>();
		
		// Filtramos los ejercicios por nivel y grupos musculares para reducir el tiempo de proceso
		List<Ejercicio> ejerciciosFiltrados = new ArrayList<>();
		
		ejercicios.forEach(ejercicio -> {
			if (ejercicio.getNivel() == nivel && gruposMusculares.contains(ejercicio.getGrupoMuscular())) {
				ejerciciosFiltrados.add(ejercicio);
			}
		});
		
		// Generamos las combinaciones
		generarCombinaciones(result, ejerciciosFiltrados, duracion, new ArrayList<>());
		
		// Imprimimos los resultados
		System.out.format("(%d) rutinas:\n", result.size());
		result.forEach(rutina -> System.out.format("- %s\n", rutina));
	}
	
	private static void generarCombinaciones(List<List<Ejercicio>> result, List<Ejercicio> ejercicios, int duracion, List<Ejercicio> aux) {
		// Caso Base 1: Si la duración es 0, hemos encontrado una combinación válida
		if (duracion == 0) {
			// Ordenamos la lista para evitar rutinas equivalentes
			List<Ejercicio> auxOrdenada = new ArrayList<>(aux);
			Collections.sort(auxOrdenada);
			// Si no está ya en la lista de resultados, la añadimos
			if (!result.contains(auxOrdenada)) {
				result.add(auxOrdenada);
			}
		// Caso Recursivo
		} else {
			// Para cada ejercicio
			for (Ejercicio e : ejercicios) {
				// Si el ejercicio no está ya en la lista y no supera la duración
				if (!aux.contains(e) && e.getDuracion() <= duracion) {
                    // Añadimos el ejercicio a la lista auxiliar				
					aux.add(e);
					// Llamamos recursivamente con la duración restante
					generarCombinaciones(result, ejercicios, duracion - e.getDuracion(), aux);
					// Eliminamos el último ejercicio añadido para probar con el siguiente
