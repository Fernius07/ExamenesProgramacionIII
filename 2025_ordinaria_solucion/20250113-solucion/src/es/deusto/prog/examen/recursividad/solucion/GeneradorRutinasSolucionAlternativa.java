...
public class GeneradorRutinasSolucionAlternativa {
	...
	public static void generarRutinas(List<Ejercicio> ejercicios, int duracion, Nivel nivel, List<GrupoMuscular> gruposMusculares) {
		List<Ejercicio> rutina = new ArrayList<Ejercicio>();
		generarRutinasRec(ejercicios, duracion, nivel, gruposMusculares, rutina, 0);
	}
	
	private static void generarRutinasRec(List<Ejercicio> ejercicios, 
			int duracion, Nivel nivel, List<GrupoMuscular> gruposMusculares, List<Ejercicio> rutina, int indice) {
		// caso base: nos hemos pasado, vamos mal, volver y probar otro ejercicio de los posibles
		if (duracion < 0) {
			return;
		}
		
		// caso base: rutina tiene la duración pedida -> y la imprimo para que la vea el usuario
		if (duracion == 0) {
			System.out.println(rutina);
		} else {
			// caso recursivo: añadimos un nuevo ejercicio a la rutina y continuamos la recursividad
			// sin embargo, para evitar repetir ejercicios, iteramos cada vez en una versión más corta de la lista
			// de ejercicios utilizando un índice
			for (int i = indice; i < ejercicios.size(); i++) {
				Ejercicio e = ejercicios.get(i);
				// solamente añadimos ejercicios que cumplan que son del nivel y trabajan alguno de los grupos musculares indicados
				if (e.getNivel() == nivel && gruposMusculares.contains(e.getGrupoMuscular())) {
					rutina.add(e);
					generarRutinasRec(ejercicios, duracion - e.getDuracion(), nivel, gruposMusculares, rutina, i + 1);
					rutina.removeLast();
				}
			}
		}
