...
public class VentanaGestorAbonos extends JFrame {
    ...
    public VentanaGestorAbonos() {
        ...
        this.addWindowListener(new WindowAdapter() {
            ...
            public void windowClosing(WindowEvent windowEvent) {
                List<Registro> registros = gestorBD.loadRegistries(LocalDate.now());
                System.out.println("\nRegistros de auditoría:");
                registros.forEach(r -> {
                    Persona persona = gestorBD.getPersona(r.getIdPersona());
                    System.out.format("%s: %s -> %s [%s]\n", persona.getNombre(), r.getAbonoAnterior(), r.getAbonoNuevo(), r.getFecha());
                });
            }
    ...
    private void actualizarPersona(Persona persona) {
    	...
    	if (gestorBD.insertRecord(persona)) {
            System.out.format("\n- Se ha insertado un registro para '%s'", persona.getNombre());
        } else {
            System.err.format("\n* No se ha podido insertar un registro para '%s'", persona.getNombre());
        }
